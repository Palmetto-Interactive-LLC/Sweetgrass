...
/**
 * Canonical memory entry types that every knowledge base starts with
 */
export const CANONICAL_MEMORY_TYPES = ["learned", "investigation"] as const;

/**
 * All first-class memory entry types recognized by the UI.
 * The set is open-ended: entries with other types are still displayed,
 * so the knowledge base can grow richer without schema breaks.
 */
...
/**
 * A memory entry type with first-class UI treatment (badge color, tab)
 */
export type KnownMemoryType = (typeof KNOWN_MEMORY_TYPES)[number];

/**
 * Memory entry type. Known types get dedicated badges and tabs; any other
 * string is accepted for forward compatibility with new categories.
 */
 ...
 * `learned`/`investigation` are kept for backward compatibility;
 * `by_type` has counts for every entry type present.
 */
...
/**
 * Facet value counts over the full (unfiltered) entry set,
...
/**
 * Optional server-side filters for GET /api/memory.
 * `since`/`until` accept unix seconds or YYYY-MM-DD (inclusive, UTC).
 */
 ...
 * `facets` is optional for compatibility with older servers.
 */
...
// ============================================================================
// bd KV Memory Types (System A — upstream bd CLI memory)
// ============================================================================

/**
 * A single key→value memory from the upstream bd CLI store
 * (`bd remember` / `bd memories --json`), persisted in the embedded beads
 * database and injected into agent sessions via `bd prime`.
 *
 * This is a DISTINCT system from the `.beads/memory/knowledge.jsonl` entries
 * (`MemoryEntry` above) — the two must never be conflated.
 */
