...
import { CheckCircle2, ChevronDown, ChevronRight, Layers, Loader2, MessageSquare, Maximize2 } from "lucide-react";
...
/**
 * Small "open full-page epic view" affordance. Reads the current projectId from
 * the URL so it doesn't require new props threaded through the card, and stops
 * click propagation so it doesn't trigger the card's onSelect.
 */
function EpicExpandLink({ epicId }: { epicId: string }) {
  const searchParams = useSearchParams();
  const projectId = searchParams.get("id");
  if (!projectId) return null;
  return (
    <Link
      href={`/epic?id=${projectId}&epic=${encodeURIComponent(epicId)}`}
      onClick={(e) => e.stopPropagation()}
      className="flex-shrink-0 rounded p-1 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-200"
