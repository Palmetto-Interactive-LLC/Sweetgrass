...
import { FolderOpen, GitPullRequest, Link2, MessageSquare, Check, X, Clock, Layers } from "lucide-react";
...
export interface BeadCardProps {
  ...
  /** Title of the parent epic, shown as a chip in task view. Omitted for standalone tasks. */
...
export function BeadCard({ bead, ticketNumber, worktreeStatus, prStatus, isSelected = false, onSelect, parentEpicTitle }: BeadCardProps) {
  ...
  return (
    ...
    >
      <CardHeader className="p-3 space-y-1.5">
        ...
        {/* Parent epic chip (task view) */}
        {parentEpicTitle && (
          <div className="flex items-center gap-1 text-xs text-purple-300/80">
            <Layers className="size-3 flex-shrink-0" aria-hidden="true" />
            <span className="truncate">{truncate(parentEpicTitle, 40)}</span>
          </div>
        )}
