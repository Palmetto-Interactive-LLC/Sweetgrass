...
pub async fn watch_beads(
    ...
    tokio::spawn(async move {
        if let Err(e) = run_watcher(project_path, beads_file, tx).await {
...
async fn run_watcher(
    ...
    while let Some(event) = notify_rx.recv().await {
        ...
        // The ledger changed on disk — drop the cached /api/beads response
        // before the debounce gate so even coalesced events leave the next
        // read fresh (sweetgrass-8u4).
